...
export default class RecipesGenerator {
  ...
  constructor(rawFolderPath: string, generatedFolderPath: string) {
    ...
    this.resultsData = openFile(`${this.rawFolderPath}/cdn/recipeResults.json`)
    ...
    this.ingredientsData = openFile(`${this.rawFolderPath}/cdn/recipeIngredients.json`)
  }
  ...
  public combineAndSaveRecipes () {
    try {
      const mountedRecipes = this.recipesData.map(recipe => {
        ...
        const ingredients = this.ingredientsData.filter(ingredient => ingredient.recipeId === recipe.id)
        const namedIngredients = ingredients.map(ingredient => {
          ...
          if (collectibleResource) {
            ...
            const resultRecipe = this.resultsData.find(result => result.productedItemId === ingredientId)
            if (resultRecipe) {
        ...
        const result = this.resultsData.find(result => result.recipeId === recipe.id)
